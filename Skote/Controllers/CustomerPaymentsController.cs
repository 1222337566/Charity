...
using InfrastructureManagmentWebFramework.Models.CustomerPayments;
...
namespace Skote.Controllers
{
    ...
    public class CustomerPaymentsController : Controller
    {
        ...
        public async Task<IActionResult> Index(Guid? customerId, CancellationToken ct)
        {
            ...
            var rows = customers.Select(c => new CustomerBalanceRowVm
            {
                CustomerId = c.Id,
            ...
            var vm = new CustomerPaymentPageVm
            {
                CustomerId = customerId,
                Rows = rows,
                TotalReceivable = rows.Where(x => x.Balance > 0).Sum(x => x.Balance)
            };
            ...
            if (customerId.HasValue)
            {
                ...
                vm.Invoices = await _db.Set<SalesInvoice>()
                    ...
                    .Select(x => new CustomerInvoiceBalanceRowVm
                    {
                        Id = x.Id,
        ...
        public async Task<IActionResult> Receive(Guid customerId, Guid? invoiceId, CancellationToken ct)
        {
            ...
            {
                vm.SuggestedAmount = Math.Max(0, vm.CurrentBalance);
                vm.Amount = vm.SuggestedAmount;
            }
        ...
        private async Task FillReceiveVmAsync(CustomerReceiveVm vm, CancellationToken ct)
        {
            ...
            vm.CurrentBalance = debit - credit;
